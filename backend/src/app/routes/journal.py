...
from app.services.ai_prompt_service import generate_prompt
...
@router.get("/ai-prompt", response_model=JournalPromptResponse)
async def get_ai_prompt(current_user: User = Depends(get_current_user)):
    """
    Return one journaling prompt. When GROQ_API_KEY is configured the prompt is
    generated by Groq's free-tier Llama model; otherwise we return a curated
    static prompt. Auth-required to keep the (rate-limited) endpoint internal.
    """
    text, source = await generate_prompt()
    return JournalPromptResponse(prompt=text, source=source)
